...
#[ignore]
async fn e2e_test_cannot_cancel_wrong_state(e2e_fixture: &E2EFixture) {
    run_e2e_test(
        "order wrong state",
        |test_context| async move {
            ...
            let response = request_cancel_order_transaction(
                &config,
                &reqwest_client,
                &bearer_token,
            ...
            print_with_time(
                "[=] [Cancel Order] - Assert Cancellation requested failed".to_string(),
            );
            let body_response =
...
#[rstest]
#[tokio::test(flavor = "multi_thread")]
async fn e2e_test_cannot_cancel_not_found(e2e_fixture: &E2EFixture) {
    run_e2e_test(
        "order not found",
        |test_context| async move {
            let config = test_context.config;
            ...
            ////////////////////////////////////////////////////////////////////////
            // Step 2 - [Cancel order] - Cancel the given order
            // Call the cancel function to validate not found.
...
#[rstest]
#[tokio::test(flavor = "multi_thread")]
async fn e2e_test_cancel_order_with_no_auth(e2e_fixture: &E2EFixture) {
    run_e2e_test(
        "No Auth",
        |test_context| async move {
            let config = test_context.config;
            ...
            print_with_time("[=] [Cancel Order] - Assert Response".to_string());
