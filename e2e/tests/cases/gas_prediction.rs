...
const END_POINT: &str = "Gas prediction fees";

#[rstest]
#[case::missing_auth_token("")]
#[case::invalid_auth_token("0")]
#[tokio::test(flavor = "multi_thread")]
async fn e2e_test_gas_prediction_bad_auth_token(
    ...
    print_with_time(format!(
        "[=] [{}] - Query gas prediction - testing authorization_token {}",
        END_POINT, auth_token
    ));
    let response = get_gas_price_prediction(
        config,
        reqwest_client,
        auth_token,
